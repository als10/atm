...
#function that formats and prints a message given as parameter in a good
#looking manner
...
def login(acc):
    ...
    while True:
        ...
        if user in users:
            ...
            #the loop checks if the password entered is correct
            #if incorrect passowrd is given 3 times, loop is exited
            while count < 3:
...
def deposit_funds(user, acc):
    while True:
        ...
        #prevents the ValueError in case incorrect data type is given as input
        try:
            funds = int(funds)
            break
        except ValueError:
